...
async def openurl(bc, url: Annotated[str, "The URL to navigate to. Value must include the protocol (http:// or https://)."],
    ...
    try:
        ...
        if page.url == url:
            logger.debug(f"Current page URL is the same as the new URL: {url}. No need to refresh.")
...
def ensure_protocol(url: str) -> str:
    ...
    if not url.startswith(('http://', 'https://')):
        ...
        logger.debug(f"Added 'https://' protocol to URL because it was missing. New URL is: {url}")
    return url
