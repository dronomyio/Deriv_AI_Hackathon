...
async def entertext(bc: PlaywrightManager, entry: Annotated[EnterTextEntry, "An object containing 'query_selector' (DOM selector query using mmid attribute e.g. [mmid='114']) and 'text' (text to enter on the element)."]) -> Annotated[str, "Explanation of the outcome of this operation."]:
    """
    ...
    result = await do_entertext(browser_manager, page, query_selector, text_to_enter)
...
async def do_entertext(bc, page: Page, selector: str, text_to_enter: str, use_keyboard_fill: bool=True):
    """
    ...
    try:
        ...
        if use_keyboard_fill:
            ...
            await press_key_combination(bc, "Control+A")
            await asyncio.sleep(0.1)
            await press_key_combination(bc, "Backspace")
            await asyncio.sleep(0.1)
...
async def bulk_enter_text(
    bc,
    entries: Annotated[List[dict[str, str]], "List of objects, each containing 'query_selector' and 'text'."] 
