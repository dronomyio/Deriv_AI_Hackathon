...
ChartSeek integrates **Opik** (open-source LLM observability platform) to provide comprehensive tracing, monitoring, and evaluation of all AI operations. This enables performance debugging, quality assessment, and continuous improvement of search accuracy.

### Traced Operations

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    CHARTSEEK OBSERVABILITY ARCHITECTURE                      │
└─────────────────────────────────────────────────────────────────────────────┘
...
User Search: "double bottom pattern"
                    ...
                    ▼
     ┌──────────────────────────┐
     │    FastAPI Endpoint      │ ◄─── @trace("chartseek_search")
     │    /api/v1/search        │
     └──────────────────────────┘
                    │
        ┌───────────┴───────────┐
...
| Component | File | Decorator | Metrics |
|-----------|------|-----------|---------|
| **CLIP Text Embedding** | `clip_embedding_service.py` | `@trace_clip_text` | Latency, query length |
| **CLIP Image Embedding** | `clip_embedding_service.py` | `@trace_clip_image` | Latency, frame count |
| **Weaviate Search** | `query_weaviate.py` | `@trace_search` | Latency, hit count, collection |
| **Search Endpoint** | `main.py` | `trace_span()` | E2E latency, result counts |
...
|----------|-------------|
| `GET /api/v1/health` | Returns observability status |
| `GET /api/v1/metrics` | Returns collected latency metrics |

**Example Response:**
```json
{
  "clip_text_embed": {"count": 150, "mean": 42.3, "min": 28.1, "max": 89.2},
  "weaviate_search": {"count": 150, "mean": 78.5, "min": 45.0, "max": 156.3},
  "search_e2e": {"count": 150, "mean": 163.2, "min": 98.5, "max": 312.7}
}
```

### Opik Dashboard

Access the Opik dashboard at **http://localhost:5173** to view:

1. **Traces View**: All search requests with latency breakdown
2. **Projects View**: Separate projects for different environments
3. **Metrics View**: Aggregated latency and throughput charts
4. **Evaluation View**: Test case pass/fail rates

### Quick Start

```bash
# Build with observability support
docker-compose build --no-cache cortex_on

# Start all services (including Opik)
docker-compose up -d

# Watch logs
docker-compose logs -f cortex_on opik
...
| `OPIK_URL_OVERRIDE` | `http://opik:5173/api` | Opik backend URL |
| `OPIK_WORKSPACE` | `default` | Workspace name |
| `OPIK_PROJECT_NAME` | `chartseek` | Project name in dashboard |

### Evaluation Framework

ChartSeek includes built-in evaluation helpers for assessing search quality:

```python
from observability import evaluate_search_quality
...
|--------|-------------|--------|
| Visual hit rate | % searches with visual matches | > 70% |
| Transcript hit rate | % searches with transcript matches | > 80% |
| Average relevance score | Mean (1 - distance) across results | > 0.5 |
| Zero-result rate | % searches returning no results | < 5% |
