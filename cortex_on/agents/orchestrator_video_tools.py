...
async def vector_index_task(
    ...
    try:
        ...
        # Call the existing weaviate_ingest.py script
        import subprocess
        
        cmd = [
            "python3",
            "/app/weaviate_ingest.py",
            "--json", json_path,
            "--video-id", video_id,
...
async def keyframe_analysis_task(
    ...
    try:
        ...
        # Call the existing keyframes_describe.py script
        import subprocess
        
        cmd = [
            "python3",
            "/app/keyframes_describe.py",
            "--clip", video_path,
            "--out", output_path,
            "--fps", str(fps),
            "--k", str(k),
